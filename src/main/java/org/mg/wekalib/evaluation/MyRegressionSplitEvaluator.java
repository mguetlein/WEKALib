 ...
 */
public class MyRegressionSplitEvaluator implements SplitEvaluator, OptionHandler, AdditionalMeasureProducer,
		RevisionHandler
{
	 ...
	 * No args constructor.
	 */
	public MyRegressionSplitEvaluator()
	{
	...
	public double getMeasure(String additionalMeasureName)
	{
		if (m_Template instanceof AdditionalMeasureProducer)
		{
			...
			{
				throw new IllegalArgumentException("ClassifierSplitEvaluator: " + "Can't return result for measure, "
		...
		{
			throw new IllegalArgumentException("ClassifierSplitEvaluator: " + "Can't return value for : "
					+ additionalMeasureName + ". " + m_Template.getClass().getName() + " "
	...
	public String[] getResultNames()
	{
		...
		resultNames[current++] = "Spearman_correlation";
	...
	public Object[] getResult(Instances train, Instances test) throws Exception
	{
		...
		{
			public MyEvaluation(Instances train) throws Exception
			{
				super(train);
			}

			public double spearmanCorrelation()
			{
				...
				return sp.correlation(actual, predicted);
			}
		}
		...
		logDensity = new double[predictions.length];
		if (m_Classifier instanceof ConditionalDensityEstimator)
		{
			for (int i = 0; i < logDensity.length; i++)
				logDensity[i] = ((ConditionalDensityEstimator) m_Classifier).logDensity(test.get(i), predictions[i]);
		}
		if (canMeasureCPUTime)
		{
		...
		result[current++] = new Double(eval.spearmanCorrelation());
