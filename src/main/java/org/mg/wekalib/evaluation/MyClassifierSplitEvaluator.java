...
public class MyClassifierSplitEvaluator implements SplitEvaluator, OptionHandler, AdditionalMeasureProducer,
		RevisionHandler
{
	 ...
	 * No args constructor.
	 */
	public MyClassifierSplitEvaluator()
	{
	...
	public Enumeration<Option> listOptions()
	{
		...
		newVector
				.addElement(new Option("\tThe index of an attribute to output in the\n"
						+ "\tresults. This attribute should identify an\n"
						+ "\tinstance in order to know which instances are\n"
						+ "\tin the test set of a cross validation. if 0\n" + "\tno output (default 0).", "I", 1,
						"-I <index>"));
	...
	public Object[] getResult(Instances train, Instances test) throws Exception
	{
		...
		probability = new double[predictions.length];
		for (int i = 0; i < predictions.length; i++)
			probability[i] = m_Classifier.distributionForInstance(test.get(i))[(int) predictions[i]];
